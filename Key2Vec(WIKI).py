...
def score1(x,y):
    ent_noun_split_x = x.replace('-', ' ').split(' ')
    ent_x = 0
    for sp_x in ent_noun_split_x:
        ent_x += model[sp_x]
    if len(ent_noun_split_x)==0:
        avg_ent_x=0
    else:
        avg_ent_x = ent_x / len(ent_noun_split_x)
    ent_noun_split_y = y.replace('-', ' ').split(' ')
    ent_y = 0
    for sp_y in ent_noun_split_y:
        ent_y += model[sp_y]
    if len(ent_noun_split_y)==0:
        avg_ent_y=0
    else:
        avg_ent_y = ent_y / len(ent_noun_split_y)
    x_vec = [avg_ent_x]
    y_vec = [avg_ent_y]
...
#Loading the fasttext WIKI model
model = KeyedVectors.load_word2vec_format(os.path.join(os.getcwd(),'Embedding models','Fasttext_WIKI','wiki-news-300d-1M.vec','wiki-news-300d-1M.vec'))
...
for filename in os.listdir(pathToFiles):
        ...
        #Thematic vector representation
        themed_vector = 0
        for i in entity_and_nounphrase_list1[:]:
            try:
                ent_noun_split = i.replace('-', ' ').split(' ')
                ent = 0
                for sp in ent_noun_split:
                    ent += model[sp]
                if len(ent_noun_split)==0:
                    avg_ent=0
                else:
                    avg_ent = ent / len(ent_noun_split)
                themed_vector += avg_ent
            except KeyError:
                #print("keyError-theme",i)
                entity_and_nounphrase_list1.remove(i)
        #Candidate Keyphrase Vector Representation
        for i in entity_and_nounphrase_list[:]:
            try:
                ent_noun_split2 = i.replace('-', ' ').split(' ')
                ent2 = 0
                for sp2 in ent_noun_split2:
                    ent2 += model[sp2]
                if len(ent_noun_split2)==0:
                    avg_ent=0
                else:
                    avg_ent2 = ent2 / len(ent_noun_split2)
            except KeyError:
                #print("keyError-entity",i)
                entity_and_nounphrase_list.remove(i)
        score = {}
        for i in entity_and_nounphrase_list[:]:
            ent_noun_split2 = i.replace('-', ' ').split(' ')
        ...
        #break
print("Total no. of documnets evaluated:",count)
